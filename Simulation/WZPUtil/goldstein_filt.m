...
if nargin<4
    n_pad=round(n_win*0.25); % 零填充长度避免FFT边缘效应
end
    
[n_i,n_j]=size(ph);
n_inc=floor(n_win/4);% 窗口滑动步长
n_win_i=ceil(n_i/n_inc)-3; % 窗口个数
n_win_j=ceil(n_j/n_inc)-3; % 窗口个数
...
[X,Y]=meshgrid(x,x);
X=X+Y;
wind_func=[X,fliplr(X)];% 窗口权重
wind_func=[wind_func;flipud(wind_func)];


ph(isnan(ph))=0;
B=gausswin(7)*gausswin(7)'; % 高斯平滑核函数
